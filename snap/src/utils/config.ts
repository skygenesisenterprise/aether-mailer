...
export class ConfigManager {
  ...
  private loadConfig(): MailerConfig {
    ...
    if (existsSync(GLOBAL_CONFIG_FILE)) {
      try {
        const fileConfig = JSON.parse(readFileSync(GLOBAL_CONFIG_FILE, 'utf-8'));
    ...
    if (existsSync(PROJECT_CONFIG_FILE)) {
      try {
        const projectConfig = JSON.parse(readFileSync(PROJECT_CONFIG_FILE, 'utf-8'));
    ...
    if (existsSync(LOCAL_CONFIG_FILE)) {
      try {
        const localConfig = JSON.parse(readFileSync(LOCAL_CONFIG_FILE, 'utf-8'));
  ...
  private loadFromEnv(): Partial<MailerConfig> {
    ...
    if (process.env.MAILER_SERVER_URL) {
      envConfig.server = { ...envConfig.server, url: process.env.MAILER_SERVER_URL };
    }
    if (process.env.MAILER_SERVER_TOKEN) {
      envConfig.server = { ...envConfig.server, token: process.env.MAILER_SERVER_TOKEN };
    }
    if (process.env.MAILER_SERVER_TIMEOUT) {
      envConfig.server = { ...envConfig.server, timeout: parseInt(process.env.MAILER_SERVER_TIMEOUT) };
    }
    if (process.env.MAILER_DEFAULT_FORMAT) {
      envConfig.cli = { ...envConfig.cli, defaultFormat: process.env.MAILER_DEFAULT_FORMAT };
    }
    if (process.env.MAILER_LOG_LEVEL) {
      envConfig.cli = { ...envConfig.cli, logLevel: process.env.MAILER_LOG_LEVEL };
    }
    if (process.env.MAILER_COLORS) {
      envConfig.output = { ...envConfig.output, colors: process.env.MAILER_COLORS === 'true' };
    }
  ...
  saveConfig(useProjectConfig = true): void {
    try {
      const configDir = useProjectConfig ? PROJECT_CONFIG_DIR : GLOBAL_CONFIG_DIR;
      const configFile = useProjectConfig ? PROJECT_CONFIG_FILE : GLOBAL_CONFIG_FILE;
  ...
  initProjectConfig(): void {
    ...
    if (!existsSync(PROJECT_CONFIG_FILE)) {
      writeFileSync(PROJECT_CONFIG_FILE, JSON.stringify(DEFAULT_CONFIG, null, 2));
    }
  }
