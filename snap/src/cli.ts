...
export class CLI {
  ...
  private parseArgs(args: string[]): { command: string; subcommand?: string; args: string[]; options: any } {
    const options: any = {};
    ...
    if (command) {
      ...
      if (command.options && command.options.length > 0) {
        ...
        command.options.forEach(opt => {
          ...
          logger.log(`  --${opt.name}${alias}  ${opt.description}${required}${defaultVal}`);
        });
        logger.log('');
      }
      ...
      logger.log('Usage: mailer <command> [options]\n');
      logger.log('Commands:');
  ...
  async run(args: string[]): Promise<void> {
    try {
      ...
      if (!command) {
        ...
        logger.log('Run "mailer --help" for usage information');
        process.exit(1);
      }
