...
// ServiceStatus represents the status of a mail service
...
// Service interface defines the contract for all mail services
...
func (sm *ServiceManager) Initialize() error {
	// Load configuration
	sm.config = config.NewConfigManager().GetConfig()

	// Validate configuration
	cm := config.NewConfigManager()
	if valid, errors := cm.ValidateConfig(); !valid {
		return fmt.Errorf("configuration validation failed: %v", errors)
	}

	// Initialize database
	sm.db = database.NewDatabaseManager(cm.GetDatabaseConfig())
	...
	sm.loggerFactory = monitoring.NewLoggerFactory(cm.GetLoggingConfig())
